...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES ('usp_load_dim_date', 'Gold', 'START');
        ...
        SELECT
            ...
            LEFT(DATENAME(WEEKDAY, full_date), 3),
            DATEPART(ISO_WEEK, full_date), -- FIXED
            CASE 
            ...
            EOMONTH(full_date),
            DATEFROMPARTS(YEAR(full_date), 1, 1),
            DATEFROMPARTS(YEAR(full_date), 12, 31),
            CONVERT(CHAR(7), full_date, 120),
            CONCAT(YEAR(full_date), '-', RIGHT('0' + CAST(DATEPART(ISO_WEEK, full_date) AS VARCHAR(2)), 2))
        FROM date_series
        OPTION (MAXRECURSION 0);

        --  SUCCESS LOG
        INSERT INTO etl.etl_logs (process_name, layer, status)
        VALUES ('usp_load_dim_date', 'Gold', 'SUCCESS');
    ...
    BEGIN CATCH
        ...
        VALUES ('usp_load_dim_date', 'Gold', 'FAILED', ERROR_MESSAGE());
